...
class PostModel(Base):
    ...
    post_interactions: Mapped[List["PostInteractionsModel"]] = relationship(back_populates="post")  # type: ignore
    ...
    async def decrease_likes(cls, post_id: int, session: AsyncSession):
        stmt = update(cls).where(cls.id == post_id).values({"likes": cls.__dict__["likes"] - 1})
    ...
    async def decrease_dislikes(cls, post_id: int, session: AsyncSession):
        stmt = update(cls).where(cls.id == post_id).values({"dislikes": PostModel.__dict__["dislikes"] - 1})
        await session.execute(stmt)
        await session.commit()

    @classmethod
    async def increase_dislikes(cls, post_id: int, session: AsyncSession):
        stmt = update(cls).where(cls.id == post_id).values({"dislikes": cls.__dict__["dislikes"] + 1})
        await session.execute(stmt)
        await session.commit()
