...
class PostInteractionsModel(Base):
    ...
    @classmethod
    async def getUserInteractionsByPost(
        cls, user_id: int, post_id: int, session: AsyncSession
    ):
    ...
    async def updateState(cls, interaction_id: int, new_state: bool, session: AsyncSession):
        stmt = (
            update(cls)
            .where(cls.id == interaction_id)
            .values(is_like=new_state)
        )
        await session.execute(stmt)
        await session.commit()

    @classmethod
    async def insertNewInteraction(
        cls, user_id: int, post_id: int, state: bool, session: AsyncSession
    ):
        new_interaction = cls(
            is_like=state, post_id=post_id, user_id=user_id
        )
